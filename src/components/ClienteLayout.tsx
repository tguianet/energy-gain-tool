...
import { Home, Calculator, User, ChevronLeft, ChevronRight, Zap, LogOut, Menu, X } from 'lucide-react';
import { cn } from '@/lib/utils';

const menuItems = [
  { label: 'Início', icon: Home, path: '/app/inicio' },
  { label: 'Minha simulação', icon: Calculator, path: '/app/minha-simulacao' },
  { label: 'Meu cadastro', icon: User, path: '/app/meu-cadastro' },
];
...
export default function ClienteLayout({ children, onLogout }: ClienteLayoutProps) {
    ...
    <div className="flex min-h-screen w-full">
      ...
      >
        <div className="flex h-16 items-center gap-3 border-b border-sidebar-border px-4">
          ...
          {!collapsed && (
            <span className="text-lg font-bold text-sidebar-foreground whitespace-nowrap">EnergiaSub</span>
          )}
          <button className="ml-auto text-sidebar-foreground md:hidden" onClick={() => setMobileOpen(false)}>
            <X className="h-5 w-5" />
          </button>
        </div>
